...
| --- | --- | --- |
| GUI to Rust | Arbitrary command or path injection | Fixed typed D-Bus methods, no command/path inputs |
...
| Backup D-Bus API | Same-UID caller supplies another path/remote/snapshot or skips policy | Only strict project IDs and selected full snapshot IDs cross D-Bus; Rust reloads the private registry; repository, commands and restore root are fixed |
...
validation, session-only credentials, bounded repository-backed history and
verified selected restore. Repository lock clears the in-memory snapshot catalog;
history must be loaded again after unlock or restart. Residual
